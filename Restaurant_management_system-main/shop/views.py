...
def contact(request):
    ...
    if request.method == "POST":
        ...
        # Print name and contact to terminal
        print("Name:", name)
...
def homepage(request, amount):
    currency = 'INR'
    amount_in_paise = amount * 100  # Convert amount to paise (Razorpay accepts amount in paise)
    ...
    if request.method == "POST":
        try:
            ...
            if result is not None:
                # Capture the payment.
                amount = int(request.POST.get('amount'))  # Retrieve the amount from the form
            ...
            # If required parameters are not found in POST data.
            return HttpResponseBadRequest("Error: {}".format(e))
        except Exception as e:
...
def cancel_order(request):
    if request.method == 'POST':
        ...
        try:
            ...
            # Here, you can perform the cancellation logic as needed
            # For example:
